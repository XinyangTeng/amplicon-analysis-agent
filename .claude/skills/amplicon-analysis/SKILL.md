...
description: Parse microbiome projects, organize inputs, design and approve analyses, call auditable MCP tools, validate deterministic R outputs, and interpret structured amplicon reports. Use for three-table ASV/OTU projects, multi-experiment or ordered-stress designs, analysis-function selection, and post-run biological interpretation.
---
...
4. Call `list_amplicon_analysis_functions`, read [references/functions.md](references/functions.md), and select functions according to the design, compatibility status, and available evidence. When metadata contains multiple experiments, pass the batch column and require within-batch inference. Pass an ordered numeric gradient column for dose or stress-severity trends. Never pool batches into one inferential group test.
...
8. Call `run_amplicon_analysis` with the returned one-time token. Let MCP orchestrate the fixed R executor and registered analysis functions; do not reproduce statistical calculations in the language model.
...
- Let the expert Skill parse the project, check the experimental design, select eligible functions, set decision rules, and explain validated results.
...
- Let R functions perform numerical analysis and create primary tables and figures.
...
Use direct function IDs such as `script-alpha` in the Analysis Contract; do not add package or collection prefixes. Include only `verified` functions in a production plan unless the required conditions of a `conditional` function are explicitly satisfied. Test `registered_untested` functions on example data first. Never select a `blocked` function.
