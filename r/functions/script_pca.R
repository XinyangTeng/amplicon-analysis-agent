...
amplicon_biomarker_path <- file.path(amplicon_path, "05_biomarker")
...
message("Biomarker output directory: ", amplicon_biomarker_path)
...
colset3  <- res[[5]]
colset4  <- res[[6]]
loading_top_n <- param_int(params, "top_n", 20)

res34 <- loadingPCA.micro(ps = ps.16s, Top = loading_top_n)

p34_1 <- res34[[1]] + theme_classic()
dat34 <- res34[[2]]
if ("id" %in% names(dat34)) {
  names(dat34)[names(dat34) == "id"] <- "ASV_ID"
  dat34 <- dat34[, c("ASV_ID", setdiff(names(dat34), "ASV_ID")), drop = FALSE]
}
