...
method <- normalize_microtest_method(param_chr(params, "pair_method", "MRPP"), "MRPP")
dist_method <- param_chr(params, "distance_method", "bray")

dat <- pairMicroTest.micro(ps = ps.16s, Micromet = method, dist = dist_method)
write_sheet2(ctx$workbook, "pair_microtest_results", dat)
