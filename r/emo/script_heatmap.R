...
feature_top_n <- param_int(params, "feature_top_n", 100)
heatnum <- param_int(params, "heatnum", 30)

ps_tem <- ps.16s %>%
  ggClusterNet::scale_micro(method = "TMM") %>%
  ggClusterNet::tax_glom_wt(ranks = comp_tax_level)

id <- ps.16s %>%
  ggClusterNet::scale_micro(method = "TMM") %>%
  ggClusterNet::tax_glom_wt(ranks = comp_tax_level) %>%
  ggClusterNet::filter_OTU_ps(feature_top_n) %>%
